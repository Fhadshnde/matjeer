...
const ProductsManagement = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `https://products-api.cbc-apps.net/products?search=${searchProduct}&limit=10&page=${currentPage}`
      );
  ...
  const handleDeleteProduct = async (deletedProductId) => {
    try {
      const token = localStorage.getItem('token');
      await fetch(`https://products-api.cbc-apps.net/products/${deletedProductId}`, {
        method: "DELETE",
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      <div className="bg-[#1A1A1A]/80 border-b border-white/5 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <AddProductModal onSubmit={recievData} />
          </div>
        </div>
      </div>

      <div dir="rtl" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-3 pt-3">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product) => (
            <div
              key={product.id}
              className="product-card relative bg-[#1A1A1A] rounded-xl border border-white/5 overflow-hidden hover:border-[#5E54F2]/50 transition-all duration-300 ease-in-out"
            >
              <div className="absolute right-4 top-4 w-6 h-6 cursor-pointer hover:-translate-y-[2px] hover:scale-110 transition-transform duration-200 rounded-lg text-red-500">
                <DeleteModal id={product.id} onDelete={handleDeleteProduct} />
              </div>
              <div className="absolute left-4 top-4 w-6 h-6 cursor-pointer hover:-translate-y-[2px] hover:scale-110 transition-transform duration-200 rounded-lg text-gray-500">
                <EditProductModal productId={product.id} onEditSuccess={fetchProducts} />
              </div>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/5">
                  ...
                  <span className="text-xs text-[#94A3B8]">
                    <span className="font-bold">{product.stock}</span> منتجات متبقية
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {visiblePages.map((page, index) => (
              <button
                ...
                disabled={page === '...'}
                className={`pagination-btn px-3 py-2 rounded-lg transition-all ${
                  page === currentPage
